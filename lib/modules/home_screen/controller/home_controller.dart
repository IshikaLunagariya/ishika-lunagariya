...
class HomeController extends GetxController {
  ...
  void onInit() {
    ...
    if (Preferences.instance.prefs?.getBool("intervalSwitch") ?? false) {}
    super.onInit();
  }
  ...
  changeBrightness({required bool dimmer, required DragUpdateDetails details}) {
    if (dimmer) {
      if (details.delta.dy < 0 && details.delta.dx == 0) {
        ...
        } else if (currentSliderValue.value.isNegative || currentSliderValue.value <= 0.9) {
        ...
        if (currentSliderValue.value.toDouble() > 0.2) {
          currentSliderValue.value -= 0.01;
        } else if (currentSliderValue.value.isNegative && currentSliderValue.value.toDouble() <= 0.2) {
        ...
        } else if (currentSliderValueForColor.value.isNegative || currentSliderValueForColor.value <= 0.9) {
        ...
        if (currentSliderValueForColor.value.toDouble() > 0.1) {
          currentSliderValueForColor.value -= 0.05;
        } else if (currentSliderValueForColor.value.isNegative && currentSliderValueForColor.value.toDouble() <= 0.1) {
